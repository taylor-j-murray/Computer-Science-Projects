...
file_path = 'C:/Users/tayma/Downloads/planet_data_base.csv'
...
# Look here for field description https://github.com/OpenExoplanetCatalogue/oec_tables/blob/master/FIELDS.md

#coefficients for a,b,c,d and S_{eff, \Odot} as given in the Kopparapu equations
RG_zero_point_one = {'SeffOdot' : 0.99, 'a': 1.029 * (10 **(-4)), 'b': 1.404 * (10 **(-8)), 'c': -7.418 * ( 10 ** (-12)) ,'d': -1.713 * ( 10**(-15)) }
RG_one_point_zero = {'SeffOdot' : 1.107, 'a': 1.332 * (10 **(-4)), 'b': 1.58 * (10 **(-8)), 'c': -8.308 * ( 10 ** (-12)) ,'d': -1.931 * ( 10**(-15)) }
RG_five_point_zero = {'SeffOdot' : 1.188, 'a': 1.433 * (10 **(-4)), 'b': 1.707 * (10 **(-8)), 'c': -8.398 * ( 10 ** (-12)) ,'d': -2.084 * ( 10**(-15)) }
MG_all = {'SeffOdot' : 0.356, 'a': 6.171 * ( 10 ** (-5)), 'b': 1.698 * ( 10 ** (-9)), 'c': -3.198 * (10 ** (-12)) ,'d': -5.575 * (10 ** (-16))}
lum_sun = 3.828 * (10 ** (26))



def j_to_e_mass (j_m : float):
    """Given a planets mass is j_m-times that of Jupiters mass, this function
    returns the number N, where the planets mass is N-times that of Earths math. 
...
def lum(p):
    ...
    """
    star_rad = p['HostStarRadiusSlrRad']  # In solar units
    star_temp = p['HostStarTempK'] / 5800 # Host Star Temp put into solar units
...
def RG_coeff_setup(p):
    ...
    dict : 
        ...
        the Running Greenhouse solar efficientcy, namely['SeffOdot', 'a', 'b', 'c', 'd'] 
...
def MG_coeff_setup(p):
    ...
    dict : 
        a dictionary whose keys are the coefficients in Kopparapu's formula for 
        the Maximum Greenhouse solar efficientcy, namely ['SeffOdot', 'a', 'b', 'c', 'd'] 
        are the keys of the dictionary and the values of these keys 
        are the corresponding floats to be employed in Kopparapu's formula.
    """
    planet_jup_mass = p['PlanetaryMassJpt']
...
def RG_S_eff(p):
    """ Given a row in planet_db this function returns the Runaway Greenhouse solar efficiency as determinded by 
...
def MG_S_eff(p):
    """ Given a row in planet_db this function returns the Maximum Greenhouse solar efficiency as determinded by 
    the mass of the planet associated to this row.
...
def in_HZ(p):
    ...
    if pd.isna(RG) or pd.isna(MG) or pd.isna(a) or pd.isna(e):
        p['IsInHabitableZone'] = np.nan
    elif a < RG or a > MG:
        p['IsInHabitableZone'] = False
    else:
        b = a*(np.sqrt(1-e**2)) # define b here to save computational power. No need to find it if a<RG or a>MG
        if b < RG: 
            p['IsInHabitableZone'] = False
        else:
            p['IsInHabitableZone'] = True
    return p

    


func_list = [lum, RG_S_eff, MG_S_eff, RG_radius, MG_radius, in_HZ]
for func in func_list:
    planet_db = planet_db.apply(func, axis =1)
#creates new columns all at once

planet_db
#displays planet_db database with new columns
